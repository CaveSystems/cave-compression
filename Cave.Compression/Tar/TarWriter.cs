...
public class TarWriter : IDisposable
{
    ...
    public static TarWriter CreateTGZ(string fileName) => new(File.Create(fileName), true);

    /// <summary>Create a new uncompressed tar file (.tar).</summary>
    /// <param name="fileName">Name of the file to create.</param>
    /// <returns>Returns a new <see cref="TarWriter"/> instance.</returns>
    public static TarWriter CreateTar(string fileName) => new(File.Create(fileName), false);
