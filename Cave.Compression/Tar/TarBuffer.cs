...
/// devices. In the C# universe, the only real function that this class performs is to ensure that files have the correct "record" size, or other tars will complain.
/// <p>You should never have a need to access this class directly. TarBuffers are created by Tar IO Streams.</p>
/// </summary>
    ...
    */
    ...
    bool ReadRecord()
    {
        ...
        while (bytesNeeded > 0)
        {
            ...
            //
            // This is a broken archive. It does not follow the standard blocking algorithm. However, because we are generous, and it requires little effort, we
            // will simply ignore the error and continue as if the entire record were read. This does not appear to break anything upstream. We used to return
            // false in this case.
            //
    ...
    /// <summary>WriteFinalRecord writes the current record buffer to output any unwritten data is present.</summary>
    /// <remarks>Any trailing bytes are set to zero which is by definition correct behaviour for the end of a tar stream.</remarks>
    ...
    /// <summary>Initializes a new instance of the <see cref="TarBuffer"/> class.</summary>
    protected TarBuffer()
    {
    }
    ...
    /// <summary>Gets or sets the Blocking factor for the buffer.</summary>
    /// <value>This is the number of blocks in each record.</value>
    public int BlockFactor { get; set; } = DefaultBlockFactor;

    /// <summary>Gets or sets the current block number, within the current record, zero based.</summary>
    /// <remarks>Block numbers are zero based values.</remarks>
    /// <seealso cref="RecordSize"/>
    public int CurrentBlock { get; set; }

    /// <summary>Gets the current record number.</summary>
    ...
    /// <summary>Gets or sets the record size for this buffer.</summary>
    /// <value>The record size in bytes. This is equal to the <see cref="BlockFactor"/> multiplied by the <see cref="BlockSize"/>.</value>
    ...
    /// <summary>Construct TarBuffer for reading inputStream setting BlockFactor.</summary>
    /// <param name="inputStream">Stream to buffer.</param>
    ...
    /// <summary>Construct TarBuffer for writing with default BlockFactor.</summary>
    ...
    /// <summary>
    /// Determine if an archive block indicates the End of an Archive has been reached. End of archive is indicated by a block that consists entirely of null
    /// bytes. All remaining blocks for the record should also be null's However some older tars only do a couple of null blocks (Old GNU tar for one) and also
    /// partial records.
    /// </summary>
    ...
    /// <summary>Close the TarBuffer. If this is an output buffer, also flush the current block before closing.</summary>
    public void Close()
    {
        if (outputStream != null)
        {
    ...
    public void WriteBlock(byte[] block)
    {
        ...
        if (outputStream == null) throw new InvalidOperationException("OutputStream is null !");
    ...
    public void WriteBlock(byte[] buffer, int offset)
    {
        ...
        if (outputStream == null) throw new InvalidProgramException("OutputStream is null!");
