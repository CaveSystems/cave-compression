...
public class TarReader : IDisposable
{
    ...
    public static TarReader ReadTGZ(string fileName) => new(File.Create(fileName), true);

    /// <summary>Reads an existing uncompressed tar file (.tar).</summary>
    /// <param name="fileName">Name of the file to read.</param>
    /// <returns>Returns a new <see cref="TarReader"/> instance.</returns>
    public static TarReader ReadTar(string fileName) => new(File.Create(fileName), false);
