...
{
    /// <summary>
    /// The TarArchive class implements the concept of a 'Tape Archive'. A tar archive is a series of entries, each of which represents a file system object.
    /// Each entry in the archive consists of a header block followed by 0 or more data blocks. Directory entries consist only of the header block, and are
    /// followed by entries for the directory's contents. File entries consist of a header followed by the number of blocks needed to contain the file's
    /// contents. All entries are written on block boundaries. Blocks are 512 bytes long.
    ///
    ...
    ///
    /// There is currently no support for random access to tar archives. However, it seems that subclassing TarArchive, and using the TarBuffer.CurrentRecord
    /// and TarBuffer.CurrentBlock properties, this would be rather trivial.
    /// </summary>
    public class TarArchive : IDisposable
    {
        ...
        // TODO: TarArchive - Is there a better way to test for a text file? It no longer reads entire files into memory but is still a weak test! This assumes
        // that byte values 0-7, 14-31 or 255 are binary and that all non text files contain one of these values
        ...
        /// entries that are files. For directories, it will call putNextEntry(), and then, if the recurse flag is true, process each entry that is a child of
        /// the directory.
        /// </summary>
        ...
        /// operating system's concept of line ends into the UNIX line end, '\n', which is the defacto standard for a TAR archive. This makes text files
        ...
        /// <summary>
        /// Gets the archive group id. See <see cref="ApplyUserInfoOverrides">ApplyUserInfoOverrides</see> for detail on how to allow setting values on a per
        /// entry basis.
        /// </summary>
        ...
        /// <summary>
        /// Gets the archive's record size. Tar archives are composed of a series of RECORDS each containing a number of BLOCKS. This allowed tar archives to
        /// match the IO characteristics of the physical device being used. Archives are expected to be properly "blocked".
        /// </summary>
        ...
        /// <summary>
        /// Gets the archive user id. See <see cref="ApplyUserInfoOverrides">ApplyUserInfoOverrides</see> for detail on how to allow setting values on a per
        /// entry basis.
        /// </summary>
        ...
        /// <summary>
        /// The InputStream based constructors create a TarArchive for the purposes of extracting or listing a tar archive. Thus, use these constructors when
        /// you wish to extract files from or list the contents of an existing tar archive.
        /// </summary>
        ...
        ///
        /// NOTE That this method uses the <see cref="ProcessEntry"/> event to actually list the contents. If the progress display event is not set, nothing
        /// will be listed!.
        /// </summary>
        ...
        /// <summary>
        /// Set user and group information that will be used to fill in the tar archive's entry headers. This information is based on that available for the
        /// linux operating system, which is not always available on other operating systems. TarArchive allows the programmer to specify values to be used in
        /// their place. <see cref="ApplyUserInfoOverrides"/> is set to true by this call.
        /// </summary>
