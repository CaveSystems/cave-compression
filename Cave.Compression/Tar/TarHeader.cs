...
///
/// This is further complicated by a large number of programs with variations on formats One common issue is the handling of names longer than 100 characters.
...
public class TarHeader
{
    ...
    static readonly DateTime DateTime1970 = new(1970, 1, 1, 0, 0, 0, 0);
    string groupName;
    string linkName;
    string magic;
    DateTime modTime;
    ...
    /// <param name="length">
    /// The number of header bytes to update. It's formatted differently from the other fields: it has 6 digits, a null, then a space -- rather than digits, a
    /// space, then a null. The final space is already there, from checksumming.
    /// </param>
    static void GetCheckSumOctalBytes(long value, byte[] buffer, int offset, int length) => WriteOctalBytes(value, buffer, offset, length - 1);

    static int GetCTime(DateTime dateTime) => unchecked((int)((dateTime.Ticks - DateTime1970.Ticks) / TimeConversionFactor));
    ...
    internal static string DefaultUser { get; set; } = string.Empty;
    ...
    internal static string UserNameAsSet { get; set; } = string.Empty;
    ...
    {
        get => userName;
        set => userName = value is null ? "user" : value[..Math.Min(UsernameLength, value.Length)];
    }
    ...
    /// <param name="length">The number of bytes to add.</param>
    /// <returns>The next free index in the <paramref name="buffer"/>.</returns>
    public static int WriteStringBytes(StringEncoding encoding, string data, byte[] buffer, int bufferOffset, int length) => WriteBytes(encoding.Encode(data), 0, buffer, bufferOffset, length, false);
    ...
    public void WriteHeader(StringEncoding encoding, byte[] outBuffer, string? replacementName)
    {
        ...
        if (TypeFlag is TarEntryType.Character or TarEntryType.Block)
        {
