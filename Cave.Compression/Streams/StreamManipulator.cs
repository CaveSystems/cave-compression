...
/// is all safe.
///
/// There are some optimizations in this class, for example, you must never peek more than 8 bits more than needed, and you must first peek bits before you may
/// drop them. This is not a general purpose class but optimized for the behaviour of the Inflater.
///
/// authors of the original java version : John Leuner, Jochen Hoenicke.
/// </summary>
sealed class StreamManipulator
{
    ...
    /// <summary>Gets or sets the number of bits available in the bit buffer. This must be only called when a previous PeekBits() returned -1.</summary>
    ...
    public int AvailableBytes => windowEnd - windowStart + (AvailableBits >> 3);

    /// <summary>Gets a value indicating whether SetInput can be called.</summary>
    public bool IsNeedingInput => windowStart == windowEnd;
    ...
    /// <summary>
    /// Copies bytes from input buffer to output buffer starting at output[offset]. You have to make sure, that the buffer is byte aligned. If not enough bytes
    /// are available, copies fewer bytes.
    /// </summary>
    ...
    /// <summary>
    /// Gets the next n bits and increases input pointer. This is equivalent to <see cref="PeekBits"/> followed by <see cref="DropBits"/>, except for correct
    /// error handling.
    /// </summary>
    ...
    /// <summary>
    /// Get the next sequence of bits but don't increase input pointer. bitCount must be less or equal 16 and if this call succeeds, you must drop at least n -
    /// 8 bits in the next call.
    /// </summary>
    ...
    /// <summary>Add more input for consumption. Only call when IsNeedingInput returns true.</summary>
    ...
    public void SetInput(byte[] buffer, int offset, int count)
    {
        ...
        // We want to throw an ArrayIndexOutOfBoundsException early. Note the check also handles integer wrap around.
