...
/// From that document:
///
/// "ADLER32 (Adler-32 checksum) This contains a checksum value of the uncompressed data (excluding any dictionary data) computed according to Adler-32
/// algorithm. This algorithm is a 32-bit extension and improvement of the Fletcher algorithm, used in the ITU-T X.224 / ISO 8073 standard.
///
/// Adler-32 is composed of two sums accumulated per byte: s1 is the sum of all bytes, s2 is the sum of all s1 values. Both sums are done modulo 65521. s1 is
/// initialized to 1, s2 to zero. The Adler-32 checksum is stored as s2*65536 + s1 in most- significant-byte first (network) order."
///
...
///
/// The modulo on unsigned long accumulators can be delayed for 5552 bytes, so the modulo operation time is negligible. If the bytes are a, b, c, the second sum
/// is 3a + 2b + c + 3, and so is position and order sensitive, unlike the first sum, which is just a checksum. That 65521 is prime is important to avoid a
/// possible large class of two-byte errors that leave the check unchanged. (The Fletcher checksum uses 255, which is not prime and which also makes the
...
public sealed class Adler32 : IZipChecksum
{
    ...
    /// <summary>Gets the Adler32 data checksum computed so far.</summary>
    ...
    /// <summary>Resets the Adler32 data checksum as if no update was ever called.</summary>
    public void Reset() => checkValue = 1;

    /// <summary>Updates the checksum with the byte b.</summary>
    ...
    /// <summary>Updates the Adler32 data checksum with the bytes taken from a block of data.</summary>
    ...
    /// <summary>Update Adler32 data checksum based on a portion of a block of data.</summary>
    ...
    public void Update(byte[] buffer, int offset, int count)
    {
        ...
        while (count > 0)
        {
            // We can defer the modulo operation: s1 maximally grows from 65521 to 65521 + 255 * 3800 s2 maximally grows by 3800 * median(s1) = 2090079800 < 2^31
            var n = 3800;
            if (n > count)
            {
                n = count;
