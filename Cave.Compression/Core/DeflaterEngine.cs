...
/// <summary>Low level compression engine for deflate algorithm which uses a 32K sliding window with secondary compression from Huffman/Shannon-Fano codes.</summary>
/// <remarks>
/// DEFLATE ALGORITHM:
///
/// The uncompressed stream is inserted into the window array. When the window array is full the first half is thrown away and the second half is copied to the beginning.
///
/// The head array is a hash table. Three characters build a hash value and they the value points to the corresponding index in window of the last string with
/// this hash. The prev array implements a linked list of matches with the same hash: prev[index &amp; WMASK] points to the previous index with the same hash.
/// </remarks>
sealed class DeflaterEngine
{
    ...
    /// <summary>
    /// Hashtable, hashing three characters to an index for window, so that window[index]..window[index+2] have this hash code. Note that the array should
    ...
    /// points to the previous index that has the same hash code as the string starting at index. This way entries with the same hash code are in a linked list.
    /// Note that the array should really be unsigned short, so you need to and the values with 0xffff.
    /// </summary>
    readonly short[] prev;

    /// <summary>This array contains the part of the uncompressed stream that is of relevance. The current character is indexed by strstart.</summary>
    ...
    /// <summary>ins_h: Hash index of string to be inserted.</summary>
    int hashIndex;
    ...
    /// <summary>Length of best match.</summary>
    int matchLen;
    ...
    /// <summary>Set if previous match exists.</summary>
    bool prevAvailable;
    ...
    bool FindLongestMatch(int curMatch)
    {
        ...
        do
        {
            ...
            // scan is set to strstart+1 and the comparison passed, so scanMax - scan is the maximum number of bytes we can compare. below we compare 8 bytes at
            // a time, so first we compare (scanMax - scan) % 8 bytes, so the remainder is a multiple of 8
    ...
    /// <summary>Inserts the current string in the head hash and returns the previous value for this hash.</summary>
    ...
    */
    ...
    public DeflaterEngine(DeflaterPending pending)
    {
        ...
        // We start at index 1, to avoid an implementation deficiency, that we cannot build a repeat pattern at index 0.
    ...
    /// <summary>Gets current value of Adler checksum.</summary>
    public int Adler => unchecked((int)adler.Value);

    /// <summary>Gets or sets the <see cref="DeflateStrategy">deflate strategy</see>.</summary>
    public DeflateStrategy Strategy { get; set; }

    /// <summary>Gets or sets the <see cref="CompressionStrength"/>.</summary>
    public CompressionStrength Strength
    {
        ...
        set
        {
            ...
            if (strength is < 0 or > 9) throw new ArgumentOutOfRangeException(nameof(value));
    ...
    /// <summary>Gets or sets total data processed.</summary>
    ...
    public bool Deflate(bool flush, bool finish)
    {
        ...
        do
        {
            ...
            {
                DeflaterConstants.DeflateStored => DeflateStored(canFlush, finish),
                DeflaterConstants.DeflateFast => DeflateFast(canFlush, finish),
                DeflaterConstants.DeflateSlow => DeflateSlow(canFlush, finish),
    ...
    /// <summary>Determines if more <see cref="SetInput">input</see> is needed.</summary>
