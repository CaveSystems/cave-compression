using System;
using System.IO;
using Cave.Compression.Streams;

namespace Cave.Compression.Core;

/// <summary>Huffman tree used for inflation.</summary>
sealed class InflaterHuffmanTree
{
    #region Private Fields

    const int MaxBitLength = 15;

    short[] tree = [];

    #endregion Private Fields

    #region Private Methods

    void BuildTree(byte[] codeLengths)
    {
        var blCount = new int[MaxBitLength + 1];
        var nextCode = new int[MaxBitLength + 1];

        for (var i = 0; i < codeLengths.Length; i++)
        {
            int bits = codeLengths[i];
            if (bits > 0)
            {
                blCount[bits]++;
            }
        }

        var code = 0;
        var treeSize = 512;
        for (var bits = 1; bits <= MaxBitLength; bits++)
        {
            nextCode[bits] = code;
            code += blCount[bits] << (16 - bits);
            if (bits >= 10)
            {
                /* We need an extra table for bit lengths >= 10. */
                var start = nextCode[bits] & 0x1ff80;
                var end = code & 0x1ff80;
                treeSize += (end - start) >> (16 - bits);
            }
        }

        /* -jr comment this out! doesnt work for dynamic trees and pkzip 2.04g
                    if (code != 65536)
                    {
                        throw new SharpZipBaseException("Code lengths don't add up properly.");
                    }
        */
        /* Now create and fill the extra tables from longest to shortest
        * bit len.  This way the sub trees will be aligned.
        */
        tree = new short[treeSize];
        var treePtr = 512;
        for (var bits = MaxBitLength; bits >= 10; bits--)
        {
            var end = code & 0x1ff80;
            code -= blCount[bits] << (16 - bits);
            var start = code & 0x1ff80;
            for (var i = start; i < end; i += 1 << 7)
            {
                tree[DeflaterHuffman.BitReverse(i)] = (short)((-treePtr << 4) | bits);
                treePtr += 1 << (bits - 9);
            }
        }

        for (var i = 0; i < codeLengths.Length; i++)
        {
            int bits = codeLengths[i];
            if (bits == 0)
            {
                continue;
            }

            code = nextCode[bits];
            int revcode = DeflaterHuffman.BitReverse(code);
            if (bits <= 9)
            {
                do
                {
                    tree[revcode] = (short)((i << 4) | bits);
                    revcode += 1 << bits;
                }
                while (revcode < 512);
            }
            else
            {
                int subTree = tree[revcode & 511];
                var treeLen = 1 << (subTree & 15);
                subTree = -(subTree >> 4);
                do
                {
                    tree[subTree | (revcode >> 9)] = (short)((i << 4) | bits);
                    revcode += 1 << bits;
                }
                while (revcode < treeLen);
            }

            nextCode[bits] = code + (1 << (16 - bits));
        }
    }

    #endregion Private Methods

    #region Internal Properties

    /// <summary>Gets the distance tree.</summary>
    internal static InflaterHuffmanTree DefDistTree { get; }

    /// <summary>Gets the literal length tree.</summary>
    internal static InflaterHuffmanTree DefLitLenTree { get; }

    #endregion Internal Properties

    #region Public Constructors

    static InflaterHuffmanTree()
    {
        try
        {
            var codeLengths = new byte[288];
            var i = 0;
            while (i < 144)
            {
                codeLengths[i++] = 8;
            }

            while (i < 256)
            {
                codeLengths[i++] = 9;
            }

            while (i < 280)
            {
                codeLengths[i++] = 7;
            }

            while (i < 288)
            {
                codeLengths[i++] = 8;
            }

            DefLitLenTree = new InflaterHuffmanTree(codeLengths);

            codeLengths = new byte[32];
            i = 0;
            while (i < 32)
            {
                codeLengths[i++] = 5;
            }

            DefDistTree = new InflaterHuffmanTree(codeLengths);
        }
        catch (Exception ex)
        {
            throw new InvalidDataException("InflaterHuffmanTree: static tree length illegal", ex);
        }
    }

    /// <summary>Initializes a new instance of the <see cref="InflaterHuffmanTree"/> class.</summary>
    /// <param name="codeLengths">the array of code lengths.</param>
    public InflaterHuffmanTree(byte[] codeLengths) => BuildTree(codeLengths);

    #endregion Public Constructors

    #region Public Methods

    /// <summary>Reads the next symbol from input. The symbol is encoded using the huffman tree.</summary>
    /// <param name="input">input the input source.</param>
    /// <returns>the next symbol, or -1 if not enough input is available.</returns>
    public int GetSymbol(StreamManipulator input)
    {
        int lookahead, symbol;
        if ((lookahead = input.PeekBits(9)) >= 0)
        {
            if ((symbol = tree[lookahead]) >= 0)
            {
                input.DropBits(symbol & 15);
                return symbol >> 4;
            }

            var subtree = -(symbol >> 4);
            var bitlen = symbol & 15;
            if ((lookahead = input.PeekBits(bitlen)) >= 0)
            {
                symbol = tree[subtree | (lookahead >> 9)];
                input.DropBits(symbol & 15);
                return symbol >> 4;
            }
            else
            {
                var bits = input.AvailableBits;
                lookahead = input.PeekBits(bits);
                symbol = tree[subtree | (lookahead >> 9)];
                if ((symbol & 15) <= bits)
                {
                    input.DropBits(symbol & 15);
                    return symbol >> 4;
                }
                else
                {
                    return -1;
                }
            }
        }
        else
        {
            var bits = input.AvailableBits;
            lookahead = input.PeekBits(bits);
            symbol = tree[lookahead];
            if (symbol >= 0 && (symbol & 15) <= bits)
            {
                input.DropBits(symbol & 15);
                return symbol >> 4;
            }
            else
            {
                return -1;
            }
        }
    }

    #endregion Public Methods
}
